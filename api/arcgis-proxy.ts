const UPSTREAM = "https://opendata.arcgis.com";
...
/** ArcGIS Hub download API only (SSRF guard). */
function allowedArcgisPath(p: string): boolean {
  if (!p.startsWith("/api/v3/datasets/")) return false;
  return p.includes("/downloads/data");
}
...
export default async function handler(req: ProxyReq, res: ProxyRes): Promise<void> {
  ...
  if (typeof p !== "string" || !allowedArcgisPath(p)) {
  ...
  try {
    ...
    console.error("[api/arcgis-proxy]", e);
